...
const Payment = ({
  ...
  const [amount, setAmount] = useState(Math.round(cartPrice * 100) / 100);
  useEffect(() => {
    setAmount(Math.round(cartPrice * 100) / 100);
  }, [cartPrice]);
  useEffect(() => {
    const makeRequest = () => {
      ...
      setStatusTitle("Order Confirmation");
      setStatusBody(`Order Number: 132`);
      setCart([]);
      setNumMealsSelected(0);
      setCartPrice(0);
      setMealNumbers(new Array(mealNumbers.length).fill(0));
    };
  ...
  return (
    <>
      ...
      <StripeCheckout
        ...
        shippingAddress
        billingAddress
        description={`Total: $${Math.round(cartPrice * 100) / 100}`}
        amount={Math.round(Math.round(cartPrice) / 100) * 100}
        // image={logo}
        token={onToken}
